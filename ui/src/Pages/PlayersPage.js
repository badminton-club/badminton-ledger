...
import {
    ...
    Spinner, Alert, InputGroup, Dropdown, DropdownButton
...
function PlayersPage() {
    ...
    const fetchAdditionalSelectedPlayerDetails = useCallback(async (playerId) => {
        ...
        if (!db) { setPlayerDetailsError("Firestore not initialized."); setIsLoadingPlayerDetails(false); return; }
        try {
    ...
    const fetchAttendedSessions = useCallback(async () => {
        ...
        setSessionsError('');
        if (!db) { setSessionsError("Firestore not initialized."); setIsLoadingSessions(false); return; }
        try {
            ...
            const playerSessions = allSessionsInMonth.filter(session =>
                session.players && Array.isArray(session.players) &&
                session.players.some(attendee => attendee.userId === selectedPlayerId)
            );
    ...
    const handleBalanceAdjustmentSubmit = async (e) => {
        ...
        if (!db) { setBalanceUpdateError("Firestore not initialized."); setIsUpdatingBalance(false); return; }
    ...
    const handleAddNewPlayer = async (newPlayerDataFromModal) => {
        if (!db) { throw new Error("Database not available. Cannot add player."); }
        try {
        ...
        <Container fluid className="mt-4">
            ...
            <Row>
                    ...
                    <Card>
                        ...
                        <Card.Body>
                            <Form.Control
                                ...
                                className="mb-2"
                            />
                            {playersStatus === 'loading' && playersList.length === 0 && <div className="text-center"><Spinner animation="border" size="sm" /></div>}
                            ...
                            {playersStatus === 'succeeded' && filteredPlayers.length === 0 && !searchTerm && playersList.length > 0 && (
                                <p className="text-muted small">Showing all players. Type to search.</p>
                            )}
                            {playersStatus === 'succeeded' && playersList.length === 0 && !searchTerm && !playersError && (
                                <p className="text-muted small">No players in the system yet.</p>
                            )}
                ...
                <Col md={8}>
                    {!selectedPlayer && (
                        <Card>
                            <Card.Body className="text-center text-muted" style={{ minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                Please select a user from the list to view their details, or add a new user.
                            </Card.Body>
                        </Card>
                    )}
                    {selectedPlayerId && isLoadingPlayerDetails && (
                        <div className="text-center mt-5"><Spinner animation="border" /><p>Loading user details...</p></div>
                    )}
                        ...
                        <>
                            <Card className="mb-3">
                                ...
                                <Card.Body>
                                    ...
                                    <p><strong>Description:</strong> {selectedPlayer.description || 'N/A'}</p>
                                    <p><strong>User ID:</strong> {selectedPlayer.id}</p>
                                    <hr />
                                    <h5>Quick Stats (Placeholder)</h5>
                                    <p>Total Sessions Attended (All Time): {selectedPlayer.attendedSessionIds?.length || 0}</p>
                                    ...
                                    <Row>
                                        <Col md={6}>
                                            <h4>Current Balance:
                                                ...
                                                {selectedPlayer.balance > 0 && <small className="text-muted"> (Credit)</small>}
                                                {selectedPlayer.balance < 0 && <small className="text-muted"> (Owes)</small>}
                                            </h4>
                                            {lastReloadInfo && (
                                                <p className="small text-muted">
                                                    Last credit: ${lastReloadInfo.amount.toFixed(2)} on {format(lastReloadInfo.date, 'MMM d, yyyy')} ({lastReloadInfo.description || lastReloadInfo.type})
                                                </p>
                                            )}
                                        ...
                                        <Col md={6}>
                                            ...
                                            {balanceUpdateError && <Alert variant="danger" size="sm">{balanceUpdateError}</Alert>}
                                            <Form onSubmit={handleBalanceAdjustmentSubmit}>
                                                <InputGroup className="mb-2">
                                                    <DropdownButton
                                                        variant="outline-secondary"
                                                        title={balanceAdjustment.type === 'credit' ? 'Add to balance (+)' : 'Deduct from balance (-)'}
                                                        id="balance-adjustment-type"
                                                    >
                                                        <Dropdown.Item onClick={() => setBalanceAdjustment(prev => ({ ...prev, type: 'credit' }))}>Add to balance  (+)</Dropdown.Item>
                                                        <Dropdown.Item onClick={() => setBalanceAdjustment(prev => ({ ...prev, type: 'debit' }))}>Deduct from balance (-)</Dropdown.Item>
                                                    </DropdownButton>
                                                    <Form.Control
                                                        type="number"
                                                        name="amount"
                                                ...
                                                <Button type="submit" variant="primary" size="sm" disabled={isUpdatingBalance}>
                                                    {isUpdatingBalance ? <Spinner as="span" animation="border" size="sm" /> : 'Update Balance'}
                                                </Button>
                                            </Form>
                                        </Col>
                                    </Row>
                            ...
                            <Card>
                                <Card.Header>
                                    <Row className="align-items-center">
                                        <Col><h5 className="mb-0">Attended Sessions - {getMonthYear(currentMonthForSessions)}</h5></Col>
                                        <Col xs="auto">
                                            <Button variant="outline-secondary" size="sm" onClick={() => setCurrentMonthForSessions(subMonths(currentMonthForSessions, 1))}>&lt; Prev</Button>
                                        </Col>
                                        ...
                                        <Col xs="auto">
                                            <Button variant="outline-secondary" size="sm" onClick={() => setCurrentMonthForSessions(addMonths(currentMonthForSessions, 1))}>Next &gt;</Button>
                                        </Col>
                                    </Row>
                                </Card.Header>
                                <Card.Body>
                                    {isLoadingSessions && <div className="text-center"><Spinner animation="border" size="sm" /></div>}
                                    {sessionsError && <Alert variant="danger" size="sm">{sessionsError}</Alert>}
                                        ...
                                        <ListGroup variant="flush" style={{ maxHeight: '200px', overflowY: 'auto' }}>
                                            {attendedSessions.map(s => (
                                                <ListGroup.Item key={s.id}>
                                                    <strong>{s.date?.toDate ? format(s.date.toDate(), 'MMM d, yyyy - p') : 'Invalid Date'}</strong>
                                                    {s.location && ` at ${s.location}`}
                                                    {/* Placeholder for link to session detail page */}
                                                    {/* <Link to={`/sessions/${s.id}`} className="float-end small">Details</Link> */}
                                                </ListGroup.Item>
                                            ))}
                                        </ListGroup>
                                    )}
