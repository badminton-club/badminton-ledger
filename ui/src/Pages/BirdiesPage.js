...
function BirdiePage() {
    ...
    useEffect(() => {
        if (selectedBatch && selectedBatch.id && !isDetailEditing) {
            const loadHistory = async () => {
                ...
                try {
                    if (
                        typeof fetchInventoryAdjustmentsForBatch !== "function" ||
                        typeof fetchSessionUsageForBirdieBatch !== "function"
                    ) {
                    ...
                    console.log("usagesData ==> ", usagesData);
                    const adjustments = (adjustmentsData || []).map((adj) => ({
                        ...adj,
                        type: "adjustment",
                        eventDate: adj.adjustmentDate?.toDate
                            ? adj.adjustmentDate.toDate()
                            : new Date(adj.adjustmentDate || 0),
                    }));

                    const usages = (usagesData || []).map((usage) => ({
                        ...usage,
                        type: "sessionUsage",
                        eventDate: usage.sessionDate?.toDate
                            ? usage.sessionDate.toDate()
                            : new Date(usage.sessionDate || 0),
                    }));
    ...
    const fetchInventory = useCallback(
        async (selectBatchIdAfterFetch = null) => {
            setIsLoading(true);
            setError(null);
            setPageLevelError("");
            if (!db) {
    ...
    const handleAddBirdieBatch = async (newBatchDataFromModal) => {
        try {
            ...
            const newBatchId = await addBirdieBatch(dataToSave);
    ...
    const handleToggleDetailEdit = () => {
        if (selectedBatch) {
            setDetailFormState({
                ...
                purchaseDate:
                    selectedBatch.purchaseDate instanceof Date
                        ? selectedBatch.purchaseDate
                        : new Date(selectedBatch.purchaseDate || Date.now()),
    ...
    const handleSaveDetailEdit = async () => {
        ...
        if (!detailFormState.name?.trim()) {
            setPageLevelError("Birdie Name/Type is required.");
            return;
        }
        if (!detailFormState.purchaseDate) {
            setPageLevelError("Purchase Date is required.");
            return;
        }
        const cost = parseFloat(detailFormState.costPerTube);
        if (isNaN(cost) || cost < 0) {
            setPageLevelError("Valid Cost Per Tube (>= 0) is required.");
            return;
        }
        const tubes = parseInt(detailFormState.tubesPurchased, 10);
        if (isNaN(tubes) || tubes < 0) {
            setPageLevelError("Valid Initial Tubes Purchased (>= 0) is required.");
            return;
        }
        const birdsNum = parseInt(detailFormState.birdsPerTube, 10);
        if (isNaN(birdsNum) || birdsNum <= 0) {
            setPageLevelError("Valid Birds Per Tube (> 0) is required.");
            return;
        }
        const unopened = parseInt(detailFormState.unopenedTubesRemaining, 10);
        if (isNaN(unopened) || unopened < 0) {
            setPageLevelError("Valid Unopened Tubes (>= 0) is required.");
            return;
        }
        const openBirds = parseInt(detailFormState.birdsInOpenTube, 10);
        if (isNaN(openBirds) || openBirds < 0 || openBirds > birdsNum) {
            setPageLevelError(`Birds in Open Tube must be between 0 and ${birdsNum}.`);
            return;
        }
        if (!detailFormState.purchaserName?.trim()) {
            setPageLevelError("Purchaser Name is required.");
            return;
        }
        if (!editReason.trim()) {
            setPageLevelError("Reason for edit is required.");
            return;
        }
        ...
        try {
            ...
            const originalDataForLog = {
                ...
                purchaseDate: originalBatchDataFromDb.purchaseDate?.toDate
                    ? originalBatchDataFromDb.purchaseDate.toDate()
                    : new Date(originalBatchDataFromDb.purchaseDate),
            };

            const dataToUpdate = {
                ...detailFormState,
                purchaseDate:
                    detailFormState.purchaseDate instanceof Date
                        ? detailFormState.purchaseDate
                        : new Date(detailFormState.purchaseDate),
            };
    ...
    const renderSummaryTable = () => {
        if (isLoading && !birdieBatches.length)
            return (
            ...
            <Table striped bordered hover responsive="sm" size="sm" className="mt-2">
                <thead>
                    <tr>
                        ...
                        <th onClick={() => requestSort("costPerTube")} style={{ cursor: "pointer" }}>
                            Cost/Tube{getSortIndicator("costPerTube")}
                        </th>
                        <th onClick={() => requestSort("unopenedTubesRemaining")} style={{ cursor: "pointer" }}>
                            Unopened Tubes{getSortIndicator("unopenedTubesRemaining")}
                        </th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <Card.Body>
                                ...
                                <Col md={6}>
                                    ...
                                    <Form.Group className="mb-3" controlId="editBatchPurchaserName">
                                        ...
                                        <Form.Control
                                            type="text"
                                            name="purchaserName"
                                            value={detailFormState.purchaserName}
                                            onChange={handleDetailFormChange}
                                            required
                                        />
                                    </Form.Group>
                                    <Form.Group className="mb-3" controlId="editBatchCostPerTube">
                                        ...
                                        <Form.Control
                                            ...
                                            min="0"
                                            step="0.01"
                                            value={detailFormState.costPerTube}
                                            onChange={handleDetailFormChange}
                                            required
                                        />
                                    </Form.Group>
                                ...
                                <Col md={6}>
                                    ...
                                    <Form.Group className="mb-3" controlId="editBatchBirdsPerTube">
                                        ...
                                        <Form.Control
                                            ...
                                            min="1"
                                            step="1"
                                            value={detailFormState.birdsPerTube}
                                            onChange={handleDetailFormChange}
                                            required
                                        />
                                    </Form.Group>
                                    <Form.Group className="mb-3" controlId="editBatchUnopenedTubes">
                                        ...
                                        <Form.Control
                                            type="number"
                                            name="unopenedTubesRemaining"
                                            min="0"
                                            step="1"
                                            value={detailFormState.unopenedTubesRemaining}
                                            onChange={handleDetailFormChange}
                                            required
                                        />
                                    </Form.Group>
                                    <Form.Group className="mb-3" controlId="editBatchBirdsInOpenTube">
                                        ...
                                        <Form.Control
                                            type="number"
                                            name="birdsInOpenTube"
                                            min="0"
                                            max={detailFormState.birdsPerTube || 12}
                                            step="1"
                                            value={detailFormState.birdsInOpenTube}
                                            onChange={handleDetailFormChange}
                                            required
                                        />
                                    </Form.Group>
                            ...
                            <Form.Group className="mb-3" controlId="editReason">
                                ...
                                <Form.Control
                                    ...
                                    required
                                    placeholder="e.g., Stock count correction, found damaged tube"
                                />
                            </Form.Group>
                        </Card.Body>
        ...
        return (
            <Card>
                <Card.Header className="d-flex justify-content-between align-items-center">
                    ...
                    <Button variant="outline-primary" size="sm" onClick={handleToggleDetailEdit}>
                        Edit Batch
                    </Button>
                </Card.Header>
                <Card.Body>
                    ...
                    <Row>
                        <Col md={6}>
                            <p>
                                <strong>Batch ID:</strong> {displayData.id}
                            </p>
                            <p>
                                <strong>Purchase Date:</strong>{" "}
                                {displayData.purchaseDate ? format(displayData.purchaseDate, "MMMM d, yyyy") : "N/A"}
                            </p>
                            <p>
                                <strong>Purchaser:</strong> {displayData.purchaserName || "N/A"}
                            </p>
                            <p>
                                <strong>Cost Per Tube:</strong> ${displayData.costPerTube?.toFixed(2) || "0.00"}
                            </p>
                        </Col>
                        <Col md={6}>
                            <p>
                                <strong>Initial Tubes Purchased:</strong> {displayData.tubesPurchased}
                            </p>
                            <p>
                                <strong>Birds Per Tube:</strong> {displayData.birdsPerTube}
                            </p>
                            <p>
                                <strong>Unopened Tubes Remaining:</strong> {displayData.unopenedTubesRemaining}
                            </p>
                            <p>
                                <strong>Birds in Current Open Tube:</strong> {displayData.birdsInOpenTube}
                            </p>
                            <p>
                                <strong>Total Remaining Birds:</strong> <strong>{totalRemaining}</strong>
                            </p>
                        </Col>
                    </Row>
                    <hr />
                    ...
                    {!isLoadingHistory && combinedHistory.length === 0 && (
                        <p className="text-muted">No usage or adjustments recorded for this batch.</p>
                    )}
                    {!isLoadingHistory && combinedHistory.length > 0 && (
                                ...
                                {combinedHistory.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={`${item.type}-${item.id || index}`}>
                                            <td style={rowStyle}>
                                                {item.eventDate ? format(item.eventDate, "yyyy-MM-dd HH:mm") : "N/A"}
                                            </td>
                                            <td style={rowStyle}>
            ...
            <Row>
                <Col md={5} lg={4} className="mb-3 mb-md-0">
                    <Card>
                        ...
                        <Card.Body style={{ maxHeight: "80vh", overflowY: "auto" }}>{renderSummaryTable()}</Card.Body>
                    </Card>
                </Col>
                <Col md={7} lg={8}>
                    {renderDetailView()}
